...
import { Wrench, CheckCircle, Clock, Package, AlertTriangle } from 'lucide-react';
...
export const EngineeringDashboard: React.FC = () => {
  ...
  const loadData = () => {
    ...
    if (savedItems) {
      ...
      const engineeringItems = items.filter(item => item.department === 'engineering_auxiliary');
      setClearanceItems(engineeringItems);
    }
    ...
    const updatedItems = clearanceItems.map(item =>
      item.id === selectedTask.id
        ? {
            ...
            completedBy: user?.name || 'Engineering Manager',
  ...
  const assetReturnData = clearanceItems.reduce((acc: any[], item) => {
  ...
  return (
    <DashboardLayout title="Engineering & Auxiliary Department">
      <div className="space-y-6">
        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Asset Returns</CardTitle>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardContent className="p-6">
              <CustomPieChart data={statusData} title="Asset Return Status" />
            </CardContent>
          </Card>
          
          <Card>
            <CardContent className="p-6">
              <CustomBarChart data={assetReturnData} title="Employee Asset Return Progress" />
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Asset Return Tasks</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Complete Asset Return</DialogTitle>
              <DialogDescription>
                Mark this asset return as completed and add your signature
              </DialogDescription>
            </DialogHeader>
            {selectedTask && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea
                    id="remarks"
                    placeholder="Add any additional notes about asset condition or return status"
                    value={remarks}
