...
import { DollarSign, CheckCircle, Clock, CreditCard, TrendingUp } from 'lucide-react';
...
export const AccountCoordinatorDashboard: React.FC = () => {
  const { user } = useAuth();
  ...
  const loadData = () => {
    ...
    if (savedItems) {
      ...
      const accountItems = items.filter(item => item.department === 'account_coordinator');
      setClearanceItems(accountItems);
    }
    ...
    const updatedItems = clearanceItems.map(item =>
      item.id === selectedTask.id
        ? {
            ...
            completedBy: user?.name || 'Account Coordinator',
  ...
  const financialData = clearanceItems.reduce((acc: any[], item) => {
  ...
  return (
    <DashboardLayout title="Account Coordinator Dashboard">
      <div className="space-y-6">
        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Financial Clearances</CardTitle>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardContent className="p-6">
              <CustomPieChart data={statusData} title="Financial Clearance Status" />
            </CardContent>
          </Card>
          
          <Card>
            <CardContent className="p-6">
              <CustomBarChart data={financialData} title="Employee Financial Clearance Progress" />
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Financial Clearance Tasks</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Complete Financial Clearance</DialogTitle>
              <DialogDescription>
                Mark this financial clearance as completed and add your signature
              </DialogDescription>
            </DialogHeader>
            {selectedTask && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea
                    id="remarks"
                    placeholder="Add any additional notes about financial obligations or clearance status"
                    value={remarks}
