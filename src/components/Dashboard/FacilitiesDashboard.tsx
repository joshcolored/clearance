...
import { Building, CheckCircle, Clock, CreditCard, AlertTriangle } from 'lucide-react';
...
export const FacilitiesDashboard: React.FC = () => {
  ...
  const loadData = () => {
    ...
    if (savedItems) {
      ...
      const facilitiesItems = items.filter(item => item.department === 'admin_facilities');
      setClearanceItems(facilitiesItems);
    }
    ...
    const updatedItems = clearanceItems.map(item =>
      item.id === selectedTask.id
        ? {
            ...
            completedBy: user?.name || 'Facilities Manager',
  ...
  const accessData = clearanceItems.reduce((acc: any[], item) => {
  ...
  return (
    <DashboardLayout title="Admin/Facilities Dashboard">
      <div className="space-y-6">
        {/* Stats Cards */}
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium">Access Cards</CardTitle>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          <Card>
            <CardContent className="p-6">
              <CustomPieChart data={statusData} title="Facility Access Status" />
            </CardContent>
          </Card>
          
          <Card>
            <CardContent className="p-6">
              <CustomBarChart data={accessData} title="Employee Access Card Returns" />
            </CardContent>
          </Card>
        </div>
        ...
        <Card>
          <CardHeader>
            <CardTitle>Facility Access Tasks</CardTitle>
          </CardHeader>
          <CardContent>
            <Table>
              <TableHeader>
        ...
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          <DialogContent>
            <DialogHeader>
              <DialogTitle>Complete Facility Access Task</DialogTitle>
              <DialogDescription>
                Mark this facility access task as completed and add your signature
              </DialogDescription>
            </DialogHeader>
            {selectedTask && (
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <Textarea
                    id="remarks"
                    placeholder="Add any additional notes about access card return or facility clearance"
                    value={remarks}
